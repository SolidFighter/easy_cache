...
%%%
%%% @end
%%% Created : 04. May 2015 10:51 PM
%%%-------------------------------------------------------------------
-module(ec_event_logger).
-author("myang").
...
%% @end
%%--------------------------------------------------------------------
-spec delete_handler() -> any().
delete_handler() ->
  ec_event:delete_handler(?MODULE, []).
...
handle_event({create, {Key, Value}}, State) ->
  ...
  {ok, State};
handle_event({read, Key}, State) ->
  error_logger:info_msg("read(~w)~n", [Key]),
  {ok, State};
handle_event({delete, Key}, State) ->
  error_logger:info_msg("delete(~w)~n", [Key]),
  {ok, State};
handle_event({update, {Key, Value}}, State) ->
  error_logger:info_msg("update(~w, ~w)~n", [Key, Value]),
  {ok, State}.
